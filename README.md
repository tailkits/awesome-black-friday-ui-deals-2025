# Black Friday & Cyber Monday 2025 - Website Templates, UI Kits & Design Tools

Curated Black Friday & Cyber Monday deals for:

- 🌐 Website & landing page templates
- 🧩 UI kits & component libraries
- 🎨 Design tools (icons, visuals, generators, etc.)

No courses, generic SaaS, or non-design products - only stuff that helps you **build and design websites faster**.

---

## Contents

- [UI Kits & Component Libraries](#ui-kits--component-libraries)
- [Website Templates](#website-templates)
- [Design Tools](#design-tools)
- [How to Add Your Deal](#how-to-add-your-deal)
- [Rules](#rules)

---

## UI Kits & Component Libraries

Name | Description | Deal
---- | ----------- | ----
⭐ [Tailkits UI](https://tailkits.com/ui) | Modern library of over 200 customizable Tailwind components | Up to **60% OFF** with code **BLACKFRIDAY2025**
[Shadcnblocks](https://www.shadcnblocks.com/?via=tailkits) | Extra shadcn/ui blocks, components and templates for React, Next.js, Astro and Tailwind CSS | Black Friday 2025: TBA - check the site during Black Friday week for the live deal.
[Shadcn Studio](https://shadcnstudio.com/) | Open-source shadcn/ui components, blocks, templates and theme generator to ship UIs faster | **30% OFF** sitewide with code **BFCM30SS** (24-11-25 to 01-12-25)

---

## Website Templates

Name | Description | Deal
---- | ----------- | ----
[daisyUI Store](https://daisyui.com/store/) | Premium templates and UI resources built on top of daisyUI and Tailwind CSS | Use **BLACKFRIDAY** code at checkout to get **40% discount** on all products.

---

## Design Tools

Name | Description | Deal
---- | ----------- | ----
<!-- Add design tools (icons, generators, design systems, etc.) here -->

---

## How to Add Your Deal

1. **Fork** this repo.  
2. Add your product under the correct section:
   - **UI Kits & Component Libraries** - Tailwind/React/Vue/etc. UI kits, component libraries, pattern libraries.
   - **Website Templates** - landing page templates, multipage site templates, dashboards, etc.
   - **Design Tools** - design/dev tools that help build or style websites (icons, visual editors, generators).
3. Follow this format:

   ```md
   Name | Description | Deal
   [Product Name](https://example.com) | 1-2 line description | XX% OFF + coupon/code + dates
